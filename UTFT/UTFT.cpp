...
void UTFT::drawRect(int x1, int y1, int x2, int y2, byte raised, 
				byte RcolorTop, byte GcolorTop, byte BcolorTop, 
				byte RcolorBottom, byte GcolorBottom, byte BcolorBottom,
				int thickness)
{
	int tmp;
	...
	{
		if(raised)
			setColor(RcolorTop, GcolorTop, BcolorTop);
		drawHLine(x1 + i, y1 + i, x2-x1-(i*2));
		drawVLine(x1 + i, y1 + i, y2-y1-(i*2));
		if(raised)
			setColor(RcolorBottom, GcolorBottom, BcolorBottom);
		drawHLine(x1 + i, y2 - i, x2-x1-(i*2));
		drawVLine(x2 - i, y1 - i, y2-y1-(i*2));
	}
}

void UTFT::drawRoundRect(int x1, int y1, int x2, int y2, 
					byte raised, byte RcolorTop, byte GcolorTop, byte BcolorTop, 
					byte RcolorBottom, byte GcolorBottom, byte BcolorBottom,
					int thickness)
{
	int tmp;
	...
	if ((x2-x1)>4 && (y2-y1)>4)
	{
		for(int i = 0; i < thickness; i++)
		{
			if(raised)
				setColor(RcolorTop, GcolorTop, BcolorTop);
			drawPixel(x1+1+i,y1+1+i);
			drawPixel(x2-1-i,y1+1+i);
			drawHLine(x1+2+i, y1, x2-x1-4-(i*2));
			drawHLine(x1+2+i, y2, x2-x1-4-(i*2));
			if(raised)
				setColor(RcolorBottom, GcolorBottom, BcolorBottom);
			drawPixel(x1+1+i,y2-1-i);
			drawPixel(x2-1-i,y2-1-i);
			drawVLine(x1+i, y1+2, y2-y1-4-(i*2));
			drawVLine(x2+i, y1+2, y2-y1-4-(i*2));
		}
	}
}
...
void UTFT::fillRect(int x1, int y1, int x2, int y2)
{
	...
	if (orient==PORTRAIT)
	{
		for (int i=0, limit = ((y2-y1)/2)+1; i<limit; i++)
		{
	...
	{
		for (int i=0, limit = ((x2-x1)/2)+1; i<limit; i++)
		{
...
void UTFT::clrScr()
{
	...
	if (display_transfer_mode!=1)
		sbi(P_RS, B_RS);
	for (i=0, limit = ((disp_x_size+1)*(disp_y_size+1)); i<limit; i++)
	{
		if (display_transfer_mode!=1)
...
void UTFT::printChar(byte c, int x, int y, byte  isTransparent)
{
	byte i,ch;
	word j, limit, mul;
	word temp; 
	...
	if (orient==PORTRAIT)
	{
		...
		for(j=0, limit = ((cfont.x_size/8)*cfont.y_size);j<limit;j++)
		{
	...
	else
	{
		...
		for(j=0, mul = cfont.x_size/8, limit = (mul*cfont.y_size);j<limit;j+=mul)
		{
			setXY(x,y+(j/mul),x+cfont.x_size-1,y+(j/mul));
			for (int zz=mul-1; zz>=0; zz--)
			{
...
void UTFT::rotateChar(byte c, int x, int y, int pos, int deg, byte  isTransparent)
{
	byte i,j,ch;
	word temp; 
	int newx,newy;
	int mul = cfont.x_size/8;
	double radian;
	radian=deg*0.0175;  

	cbi(P_CS, B_CS);

	temp=((c-cfont.offset)*(mul*cfont.y_size))+4;
	for(j=0;j<cfont.y_size;j++) 
	{
		for (int zz=0; zz<mul; zz++)
		{
			ch=pgm_read_byte(&cfont.font[temp+zz]); 
			for(i=0;i<8;i++)
			{   
...
void UTFT::print(char *st, int x, int y, int deg, byte  isTransparent)
{
	int stl, i;
	...
	for (i=0; i<stl; i++)
		if (deg==0)
			printChar(*st++, x + (i*(cfont.x_size)), y, isTransparent);
		else
			rotateChar(*st++, x, y, i, deg, isTransparent);
}

void UTFT::print(String st, int x, int y, int deg, byte  isTransparent)
{
	char buf[st.length()+1];

	st.toCharArray(buf, st.length()+1);
	print(buf, x, y, deg, isTransparent);
}

void UTFT::printNumI(long num, int x, int y, int length, char filler, byte  isTransparent)
{
	char buf[25];
	char st[27];
	boolean neg=false;
	...
	if (num==0)
	{
		if (length!=0)
		{
			for (c=0, limit = length-1; c<limit; c++)
				st[c]=filler;
			st[c]=48;
			st[c+1]=0;
		}
	...
	else
	{
		...
		if (length>(c+neg))
		{
			for (int i=0, limit2 = length-c-neg; i<limit2; i++)
			{
				st[i+neg]=filler;
				f++;
			}
	...
	print(st,x,y, isTransparent);
}

void UTFT::printNumF(double num, byte dec, int x, int y, char divider, int length, char filler, byte  isTransparent)
{
	char buf[25];
	char st[27];
	boolean neg=false;
	...
	if (num==0)
	{
		if (length!=0)
		{
			for (c=0, limit = length-2-dec; c<limit; c++)
				st[c]=filler;
			st[c]=48;
			st[c+1]=divider;
			for (int i=0; i<dec; i++)
	...
	else
	{
		...
		if (length>(c+neg-1))
		{
			for (int i=0, limit2 = length-c-neg-1; i<limit2; i++)
			{
				st[i+neg]=filler;
				f++;
			}
...
void UTFT::drawBitmap(int x, int y, int sx, int sy, bitmapdatatype data, int scale)
{
	...
	if (scale==1)
	{
		if (orient==PORTRAIT)
		{
			...
			for (tc=0, limit = sx*sy; tc<limit; tc++)
			{
...
/**
* draws a button and if the bitmap is not NULL puts a bitmap on the left inside the button
*/
void UTFT::drawButton(int x1, int y1, int x2, int y2, char *st, bitmapdatatype data, byte isRounded, byte isPressed1)
{
	// calculated where it begins to draw the text and icon
	int x = (x2-x1)/2 - ((strlen(st)*cfont.x_size)/2);
	if(data != NULL)
		x -= 48;
		
	// roundedred are 4 pixel for side
	// draw border
	if(isPressed1)
	...
	if(isRounded)
	{
		...
		if(isPressed1)
			drawRoundRect(x1, y1, x2, y2,1, 150,150,150, 200,200,200,3);
		else
			drawRoundRect(x1, y1, x2, y2,1, 200,200,200, 150,150,150,3);
	}
	else
	{
		fillRect(x1, y1, x2, y2);
		if(isPressed1)
			drawRect(x1, y1, x2, y2,1, 150,150,150, 200,200,200,3);
		else
			drawRect(x1, y1, x2, y2,1, 200,200,200, 150,150,150,3);
	}

	setColor(0,0,0);
	int y= y1 + (((y2 - y1)/2) - (cfont.y_size/2));
	int yy= y1 + (((y2 - y1)/2) - 16);
	if(data != NULL)
	{
		// we have to draw the icon.
		drawBitmap(x1 + x + 6, yy, 32, 32, data, 1);
		setColor(0,0,0);
		print(st, x1 + x + 48, y,1,1);
	}
	else
	{
		setColor(0,0,0);
		print(st, x1 + x + 6, y,1,1);
	}
}
